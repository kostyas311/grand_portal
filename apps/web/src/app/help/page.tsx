...
const roleGuides = [
  {
    ...
    tasks: [
      ...
      'Создавать и поддерживать протоколы проверки, если команде нужен единый порядок приёмки результата.',
  ...
  },
  {
    ...
    tasks: [
      ...
      'Проверять, что результаты приложены, а протокол проверки пройден до закрытия карточки.',
  ...
  },
  {
    ...
    tasks: [
      ...
      'Следить за наполнением инструкций, компонентов, протоколов проверки и справочных разделов.',
...
const portalSections = [
  ...
  },
  {
    ...
    href: '/review-protocols',
    icon: ClipboardCheck,
    description: 'Отдельный справочник чек-листов, которые используются для проверки результата в карточках.',
    bullets: [
      'Любой пользователь может создать, отредактировать и архивировать протокол.',
      'Протокол можно прикрепить к источнику данных или к конкретной карточке.',
    ],
  },
  {
    title: 'Обращения',
...
const quickStart = [
  ...
  'Если для карточки или источника задан протокол проверки, ориентируйтесь на него при приёмке результата.',
...
const cardFlow = [
  ...
  },
  {
    title: 'На проверке',
    tone: 'review',
    text: 'Проверяющий смотрит приложенный результат, проходит протокол проверки, оставляет замечания или подтверждает завершение.',
  },
  {
    title: 'Выполнено',
    tone: 'done',
...
const reviewProtocolRules = [
  'Протоколы проверки находятся в отдельном разделе меню и открываются как обычные страницы, а не модальные окна.',
  'Протокол можно прикрепить к источнику данных. При создании карточки на основе такого источника в карточку копируется собственная версия протокола.',
  'В карточке протокол показывается только в статусе «На проверке» и располагается первым блоком содержимого.',
  'Карточку нельзя перевести в «Готово», пока в протоколе остаются незакрытые пункты. Для администратора это ограничение не действует.',
  'Если проверяющий возвращает карточку в работу с незакрытым протоколом, система автоматически добавляет в комментарий список непройденных пунктов.',
  'Во время проверки убрать протокол, инструкцию или компонент из карточки может только проверяющий или администратор.',
] as const;
    ...
    <AppLayout>
      <div className="page-container-wide">
        ...
        <section className="help-section">
          <div className="help-section-head">
            <div>
              <div className="page-kicker">Проверка</div>
              <h2 className="help-section-title">Как работают протоколы проверки</h2>
            </div>
          </div>
          <div className="help-two-column">
            <article className="help-note-card">
              ...
              <h3 className="help-note-title">Основные правила</h3>
              <ul className="help-list">
                {reviewProtocolRules.map((rule) => (
                  <li key={rule}>{rule}</li>
                ))}
              </ul>
            </article>
            ...
            <article className="help-note-card">
              ...
              <h3 className="help-note-title">Практический сценарий</h3>
              <ul className="help-list">
                <li>Откройте карточку в статусе «На проверке» и начните с блока протокола проверки.</li>
                <li>Отмечайте только действительно пройденные пункты и при необходимости возвращайте карточку в работу.</li>
                <li>Когда закрыт последний пункт, портал может сразу предложить перевести карточку в «Готово».</li>
                <li>Если протокол не нужен для конкретной карточки, его снимает проверяющий или администратор.</li>
              </ul>
            </article>
          </div>
        </section>
