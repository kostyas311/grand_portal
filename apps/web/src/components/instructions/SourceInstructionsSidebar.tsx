...
import { dataSourcesApi, instructionsApi } from '@/lib/api';
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  disabled={isAttached}
                  title={isAttached ? 'Инструкция уже прикреплена' : 'Прикрепить инструкцию'}
                >
...
export function SourceInstructionsSidebar({
  sourceId,
  sourceName,
  isOpen,
  onClose,
  ...
  const { data: linkedInstructions } = useQuery({
    queryKey: ['data-source-instructions', sourceId],
    queryFn: () => dataSourcesApi.getInstructions(sourceId),
    enabled: isOpen,
  });
  ...
  const { data: publishedInstructions } = useQuery({
    queryKey: ['published-instructions-for-source'],
    queryFn: () => instructionsApi.getPublished(),
    enabled: isOpen,
  });

  const attachMutation = useMutation({
    mutationFn: (instructionId: string) => dataSourcesApi.attachInstruction(sourceId, instructionId),
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['data-source-instructions', sourceId] }),
        queryClient.invalidateQueries({ queryKey: ['data-sources'] }),
      ]);
      toast.success('Инструкция прикреплена к источнику');
    },
    onError: (err: any) => {
      toast.error(err?.response?.data?.message || 'Не удалось прикрепить инструкцию');
    },
  });

  const detachMutation = useMutation({
    mutationFn: (instructionId: string) => dataSourcesApi.detachInstruction(sourceId, instructionId),
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['data-source-instructions', sourceId] }),
        queryClient.invalidateQueries({ queryKey: ['data-sources'] }),
      ]);
      toast.success('Инструкция откреплена от источника');
    },
    onError: (err: any) => {
            ...
            {tab === 'linked' ? (
              !(linkedInstructions || []).length ? (
                <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-5 py-8 text-center text-sm text-slate-500">
                  У источника пока нет прикреплённых инструкций.
                </div>
              ) : (
