...
### 📋 Итерация 6: Интеграция с Bitrix24
**Статус:** частично реализовано, внешняя проверка отложена

**Базовые сценарии:**
- [ ] Отправка уведомлений из `Нормбазы` в Bitrix24 по ключевым событиям карточек
- [ ] Настройка подключения к Bitrix24 в админке
- [ ] Сопоставление пользователей `Нормбазы` и Bitrix24
- [ ] Шаблоны сообщений и ссылки на карточки
- [ ] Логирование и повторная отправка при ошибках

**Предпочтительный порядок:**
1. Подключение и тест доставки
2. Уведомления по ключевым статусным событиям
3. Mapping пользователей
4. Агрегация и полировка текстов уведомлений

**Нюансы:**
- [ ] Использовать только HTTPS-доступные обработчики
- [ ] Хранить секреты вебхуков вне репозитория
- [ ] Не использовать задачи Bitrix24 как часть интеграции
- [ ] Не использовать сделки / лиды / контакты как объект синхронизации
- [ ] Bitrix24 использовать только как канал уведомлений для пользователей

**Текущая точка остановки:**
- [x] Экран настройки Bitrix24 на портале добавлен
- [x] Backend-настройки Bitrix24 добавлены
- [x] Поле `ID Bitrix24` у пользователя добавлено
- [ ] Реальный webhook не проверен
- [ ] Реальная доставка не проверена
- [ ] Проверка отложена, так как подписка Bitrix24 временно закончилась
